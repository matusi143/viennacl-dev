    ...
    namespace opencl
    {
      namespace kernels
      {
        ...
        void generate_sliced_ell_vec_mul(StringType & source, std::string const & numeric_string)
        {
          ...
          source.append("  uint num_rows   = layout_x.z; \n");

          source.append("  for(uint block_idx = get_group_id(0); block_idx < num_rows / get_local_size(0) + 1; row += get_num_groups(0)) { \n");
          source.append("    "); source.append(numeric_string); source.append(" sum = 0; \n");

          source.append("    uint row    = block_idx * local_size + local_id; \n");
          source.append("    uint offset = block_start[block_id]; \n");
          source.append("    uint num_columns = columns_per_block[block_id]; \n");
          ...
          source.append("      sum += val ? (x[index] * val); \n");
          source.append("    } \n");
